...
if [ $stage -le 4 ]; then
  ...
  dir=$PROJECTS/exp/swbd_train_phn_nj4_after_nj1_l${lstm_layer_num}_c${lstm_cell_dim}
  mkdir -p $dir
	mkdir $dir/nnet

	initial_network=$1

	echo "intial network is ($initial_network)"

	[ ! -f "$initial_network" ] && echo 'This script runs CTC on top of a trained CTC model, please provide the initial model as a parameter to the script.' && exit 1

	cp $initial_network $dir/nnet/nnet.iter0
  ...
  # Train the network with CTC. Refer to the script for details about the arguments
  steps/train_ctc_parallel_h2.sh --add-deltas true --num-sequence 20 --frame-num-limit 25000 \
    --learn-rate 0.00004 --report-step 1000 --halving-after-epoch 12 --nj 4 \
