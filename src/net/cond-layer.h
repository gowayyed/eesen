...
#ifndef EESEN_AFFINE_TRANS_COND_LAYER_H_
#define EESEN_AFFINE_TRANS_COND_LAYER_H_
...
class AffineTransformCond : public TrainableLayer {
 public:
  AffineTransformCond(int32 dim_in, int32 dim_out) 
  ...
  Layer* Copy() const { return new AffineTransformCond(*this); }
  LayerType GetType() const { return l_Affine_Transform_Cond; }
  LayerType GetTypeNonParal() const { return l_Affine_Transform_Cond; }
  ...
  void Add(BaseFloat scale, const TrainableLayer & layer_other) {
    const AffineTransformCond *other = dynamic_cast<const AffineTransformCond*>(&layer_other);
