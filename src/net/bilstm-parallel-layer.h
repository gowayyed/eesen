...
#include "net/nnet-precondition.h"
...
class BiLstmParallelPreconditioned : public BiLstmParallel {
public:
    BiLstmParallelPreconditioned(int32 input_dim, int32 output_dim) : BiLstmParallel(input_dim, output_dim)
    { }
    ~BiLstmParallelPreconditioned()
    { }

    Layer* Copy() const { return new BiLstmParallelPreconditioned(*this); }
    LayerType GetType() const { return l_BiLstm_Parallel_Preconditioned; }

    void Precondition(const CuMatrixBase<BaseFloat> &in, const CuMatrixBase<BaseFloat> &out, BaseFloat alpha, CuMatrix<BaseFloat> *in_precon, CuMatrix<BaseFloat> * out_precon) {
				if(in_precon != NULL)
				{
					CuMatrix<BaseFloat> in_tmp(in.NumRows(), in.NumCols() + 1, kUndefined);
		      in_tmp.Range(0, in.NumRows(), 0, in.NumCols()).CopyFromMat(in);
			    in_tmp.Range(0, in.NumRows(), in.NumCols(), 1).Set(1.0);
					in_precon->Resize(in_tmp.NumRows(), in_tmp.NumCols());
					PreconditionDirectionsAlphaRescaled(in_tmp, alpha, in_precon);
				}
				if(out_precon != NULL)
				{
					out_precon->Resize(out.NumRows(), out.NumCols());
	        PreconditionDirectionsAlphaRescaled(out, alpha, out_precon);
				}
}	
